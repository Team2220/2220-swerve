...
public class SanityCheck {
    ...
    public boolean isStable() {
        if (updates.size() < TARGET_POSE_UPDATES) 
            return false;
        ...
        for (int i = 1; i < updates.size(); i++) {
            ...
            double timeDiff = (end.time - end.reportedPos.latency) - (start.time - start.reportedPos.latency);
            timeDiff /= 1000;
            ...
            if (Math.abs(xDisplacement - drive.getStr()) > STABILITY_TOLERANCE)
                return false;
            ...
            if (Math.abs(yDisplacement - drive.getFwd()) > STABILITY_TOLERANCE)
                return false;
        }
